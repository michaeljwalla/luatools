...
local tableencode,tabledecode = (function() 
    ...
    local function decode(str, recursed)
        ...
        for i,v in next, str do
            ...
            if rawequal(type, "n") then
                local _start, _end = data:find(ch(info.k))
                assert(_start, "Invalid key.")
                local key = parse(data:sub(1, _end-1))
                local ending = data:find(ch(info.ea))
            ...
            elseif rawequal(type, "e") then
                ...
                assert(_start, "Invalid key.")
                local key = parse(data:sub(1, _end-1))
                local ending = data:reverse():find(ch(info.ee)) --last index of closetbl
    ...
    local function encode(tbl, nocycles, recursed)
        ...
        for i,v in next, tbl do
            ...
            if type ~= 'e' then
                str = str..ch(info[type])
                ..to(i)..ch(info.k)
                ..to(v)..ch(info.ea)--..ch(info.c)
            else
                --print()
                ...
                local newentry = ch(info.e)..to(i)..ch(info.k)..(data or "")..ch(info.ee)
                str = str..newentry
            end
